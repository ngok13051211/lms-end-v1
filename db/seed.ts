...
//         name: "Hóa học",
//         icon: "science",
//         description: "Môn hóa học phổ thông",
//       },
//       { name: "Vật lý", icon: "bolt", description: "Môn vật lý phổ thông" },
//       {
//         name: "Ngữ văn",
//         icon: "book-open",
//         description: "Môn ngữ văn phổ thông",
//       },
//     ];
...
//       // Add some subjects and education levels to the tutor
//       const subjectsList = await db.select().from(schema.subjects);
//       const levelsList = await db.select().from(schema.educationLevels);

//       // Add 2-3 random subjects to the tutor
//       const randomSubjectsCount = Math.floor(Math.random() * 2) + 2;
//       for (let j = 0; j < randomSubjectsCount; j++) {
...
//       // Add 1-2 random education levels to the tutor
//       const randomLevelsCount = Math.floor(Math.random() * 2) + 1;
//       for (let j = 0; j < randomLevelsCount; j++) {
//         const randomLevel = levelsList[Math.floor(Math.random() * levelsList.length)];
//         await db
...
//       // Create courses for the tutor (1-3 courses)
...
//             subject_id: randomSubject.id,
//             level_id: randomLevel.id,
//             title: `${randomSubject.name} cho ${randomLevel.name}`,
//             description: faker.lorem.paragraph(),
//             hourly_rate: (Math.random() * 300000 + 150000).toFixed(2),
//             teaching_mode: ['online', 'offline', 'both'][Math.floor(Math.random() * 3)],
//             status: 'active',
//             created_at: new Date(),
//             updated_at: new Date(),
//           })
...
//     // Fetch existing students and tutors to create relationships
//     const students = await db.select().from(schema.users).where(eq(schema.users.role, "student"));
//     const tutors = await db.select({ id: schema.users.id, profile_id: schema.tutorProfiles.id })
//       .from(schema.users)
//       .innerJoin(schema.tutorProfiles, eq(schema.users.id, schema.tutorProfiles.user_id))
//       .where(eq(schema.users.role, "tutor"));

//     // Fetch courses to link with booking requests
//     const courses = await db.select().from(schema.courses);

//     // Create favorite tutors relationships
//     console.log("Creating favorite tutors relationships...");
//     for (let i = 0; i < 15; i++) {
//       const randomStudent = students[Math.floor(Math.random() * students.length)];
//       const randomTutor = tutors[Math.floor(Math.random() * tutors.length)];

//       if (randomStudent && randomTutor) {
//         await db.insert(schema.favoriteTutors)
//           .values({
//             student_id: randomStudent.id,
//             tutor_id: randomTutor.profile_id,
//             created_at: new Date(),
//           })
//           .onConflictDoNothing();
//       }
...
//     const teachingModes = ['online', 'offline'];

//     const bookingRequestsData = [];
//     for (let i = 0; i < 20; i++) {
//       const randomStudent = students[Math.floor(Math.random() * students.length)];
//       const randomTutor = tutors[Math.floor(Math.random() * tutors.length)];
//       const randomCourse = courses.length > 0 ? courses[Math.floor(Math.random() * courses.length)] : null;
//       const randomStatus = bookingStatuses[Math.floor(Math.random() * bookingStatuses.length)];
//       const randomMode = teachingModes[Math.floor(Math.random() * teachingModes.length)];

//       const hourlyRate = parseFloat((Math.random() * 200000 + 150000).toFixed(2));
//       const totalHours = parseFloat((Math.random() * 10 + 1).toFixed(1));
...
//           student_id: randomStudent.id,
//           tutor_id: randomTutor.profile_id,
//           course_id: randomCourse?.id || null,
//           title: `Buổi học ${randomMode === 'online' ? 'trực tuyến' : 'trực tiếp'} ${faker.lorem.words(3)}`,
//           description: faker.lorem.paragraph(),
//           mode: randomMode,
//           location: randomMode === 'offline' ? faker.location.streetAddress() : null,
//           note: faker.lorem.sentence(),
//           meeting_url: randomMode === 'online' ? `https://meet.google.com/${faker.string.alphanumeric(10)}` : null,
//           hourly_rate: hourlyRate.toString(),
//           total_hours: totalHours.toString(),
//           total_amount: totalAmount.toString(),
//           status: randomStatus,
//           rejection_reason: randomStatus === 'rejected' ? faker.lorem.sentence() : null,
//           created_at: faker.date.recent({ days: 30 }),
//           updated_at: new Date(),
//         };

//         const [insertedRequest] = await db.insert(schema.bookingRequests)
//           .values(requestData)
//           .returning();

//         bookingRequestsData.push(insertedRequest);
//       }
//     }

//     // Create booking sessions for each request
//     console.log("Creating booking sessions...");
//     const sessionStatuses = ['pending', 'confirmed', 'completed', 'cancelled'];

//     for (const request of bookingRequestsData) {
//       const sessionsCount = Math.floor(Math.random() * 5) + 1; // 1 to 5 sessions per request

//       for (let i = 0; i < sessionsCount; i++) {
//         const randomDate = faker.date.soon({ days: 30, refDate: new Date(request.created_at) });
...
//         await db.insert(schema.bookingSessions)
//           .values({
...
async function seed() {
  try {
    ...
    const generateVietnameseEmail = (firstName, lastName) => {
      const providers = [
        "gmail.com",
        "yahoo.com",
        "hotmail.com",
    ...
    const generateAvatarUrl = () => {
      ...
      return `https://res.cloudinary.com/homitutor/image/upload/v1683241476/avatars/${avatarIds[Math.floor(Math.random() * avatarIds.length)]
        }.jpg`;
    };
    ...
    const getCourseTemplatesForSubject = (subjectName) => {
    ...
    const generateCourseDescription = (title, subjectName) => {
      const descriptions = [
    ...
    const generateIntroduction = (subjectName) => {
      const introductions = [
    ...
    const generateExperience = (subjectName) => {
      const experiences = [
