...
import { Loader2, AlertCircle, BadgeCheck, BookOpen, FileCheck, RefreshCw } from "lucide-react";
...
export default function TeachingRequestsList({ onCreateRequest }: TeachingRequestsListProps) {  // Fetch teaching requests data
    const { data: teachingRequestsData, isLoading: requestsLoading, refetch } = useQuery<any>({
    ...
    if (!teachingRequests || teachingRequests.length === 0) {
        ...
        <div className={`space-y-6 transition-all duration-300 ${isUpdated ? 'bg-primary/5 rounded-md p-4' : ''}`}>
            <div className="flex justify-between items-center">
                {teachingRequests.length > 0 && (
                    <div className="text-sm text-muted-foreground flex items-center">
                        <span>Tổng cộng: {teachingRequestsData?.total || 0} yêu cầu giảng dạy</span>
                        {isUpdated && (
                ...
                <div key={index} className="border rounded-lg p-4 bg-card">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        <div>
                            ...
                            <p className="font-medium">{request.subject?.name || "Không xác định"}</p>
                        </div>
                        <div>
                            <h3 className="text-sm font-medium text-muted-foreground">Cấp học</h3>
                            <p className="font-medium">{request.level?.name || "Không xác định"}</p>
                        </div>
                        <div>
                            <h3 className="text-sm font-medium text-muted-foreground">Ngày yêu cầu</h3>
                            <p>{request.created_at ? format(new Date(request.created_at), "dd/MM/yyyy") : "-"}</p>
                        </div>
                        <div>
                    ...
                    <div className="space-y-4 mb-4">
                        <div>
                            <h3 className="text-sm font-medium text-muted-foreground mb-1">Giới thiệu</h3>
                            <p className="text-sm whitespace-pre-wrap">{request.introduction}</p>
                        </div>
                        <div>
                            <h3 className="text-sm font-medium text-muted-foreground mb-1">Kinh nghiệm</h3>
                    ...
                    {request.certifications && (
                        <div>
                            <h3 className="text-sm font-medium text-muted-foreground mb-2">Chứng chỉ</h3>
                                ...
                                {Array.isArray(request.certifications) ? (
                                    ...
                                    (() => {
                                        try {
                                            ...
                                            return <p className="text-xs text-muted-foreground">Không thể hiển thị chứng chỉ</p>;
                                        }
                                    })()
                                ) : (
                                    <p className="text-xs text-muted-foreground">Không thể hiển thị chứng chỉ</p>
                                )}
                            </div>
                        </div>
                    )}
                ...
                <div className="flex justify-center mt-8">
                    <div className="flex items-center space-x-2">
                        <Button
                            ...
                            onClick={() => {
                                const prevPage = Math.max((teachingRequestsData?.current_page || 2) - 1, 1);
                                queryClient.prefetchQuery({
                                    queryKey: [`/api/v1/tutors/teaching-requests`, { page: prevPage }],
                                });
                            }}
                        >
                            Trang trước
                        ...
                        <div className="text-sm">
                            Trang {teachingRequestsData?.current_page} / {teachingRequestsData?.total_pages}
                        </div>
                        ...
                        <Button
                            ...
                            size="sm"
                            disabled={
                                (teachingRequestsData?.current_page || 0) >= (teachingRequestsData?.total_pages || 0)
                            }
                            onClick={() => {
                                const nextPage = (teachingRequestsData?.current_page || 0) + 1;
                                queryClient.prefetchQuery({
                                    queryKey: [`/api/v1/tutors/teaching-requests`, { page: nextPage }],
                                });
                            }}
                        >
                            Trang sau
