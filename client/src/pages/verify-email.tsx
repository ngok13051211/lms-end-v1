...
export default function VerifyEmail() {
  ...
  useEffect(() => {
    if (decodedEmail) {
      ...
      verificationService
        .sendOtp(decodedEmail)
        .then((response) => {
          if (response.success) {
            toast({
              title: "Mã OTP đã được gửi",
              description:
                "Vui lòng kiểm tra email của bạn để nhận mã xác thực.",
            });
            setSecondsLeft(60);
        ...
        .catch((error) => {
          ...
          toast({
            ...
            description:
              error.message ||
              "Đã xảy ra lỗi khi gửi OTP. Vui lòng sử dụng nút gửi lại OTP.",
            variant: "destructive",
          });
        });
    }
              ...
              <div className="flex flex-col items-center justify-center space-y-4 py-6">
                ...
                <div className="text-center">
                  ...
                  <p className="text-sm text-muted-foreground mt-2">
                    Email của bạn đã được xác minh. Đang chuyển hướng đến trang
                    đăng nhập...
                  </p>
                </div>
              </div>
                            ...
                            render={({ slots }) => (
                              <InputOTPGroup>
                                {slots.map((slot, index) => (
                                  <InputOTPSlot
                                    key={index}
                                    // @ts-ignore - Bỏ qua kiểm tra kiểu cho slot do vấn đề tương thích từ thư viện
                                    slot={slot}
