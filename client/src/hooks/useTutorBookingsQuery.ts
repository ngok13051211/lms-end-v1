...
interface UseTutorBookingsOptions {
  status?:
    | "all"
    | "pending"
    | "confirmed"
...
/**
 * A custom hook to fetch the current tutor's bookings from the backend API
 *
 ...
 */
export function useTutorBookingsQuery(options: UseTutorBookingsOptions = {}) {
  const { status } = options;
  const { user } = useSelector((state: RootState) => state.auth);

  // Build the API URL with optional status parameter
  const buildApiUrl = () => {
    const url = new URL("/api/v1/bookings/tutor", window.location.origin);
  ...
  return useQuery<Booking[]>({
    queryKey: ["tutorBookings", status || "all"],
    queryFn: async () => {
      // Check if user is logged in and is a tutor
      if (!user || user.role !== "tutor") {
        throw new Error(
          "You must be logged in as a tutor to view booking requests."
        );
      }
...
export default useTutorBookingsQuery;
