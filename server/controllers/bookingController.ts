...
export const createBooking = async (req: Request, res: Response) => {
  try {
    ...
    const hourlyRate = typeof validatedData.hourly_rate === 'string' 
...
export const updateBookingStatus = async (req: Request, res: Response) => {
  try {
    ...
    if (booking.student_id === userId) {
      ...
      if (status === "cancelled") {
        return res.status(403).json({ message: "Tutors cannot cancel bookings" });
      }
    }
...
export const addSessionNotes = async (req: Request, res: Response) => {
  try {
    ...
    if (!isStudent && !isTutor) {
      return res.status(403).json({ message: "You don't have permission to add notes to this session" });
    }
    ...
    if (booking.sessionNote) {
      [sessionNote] = await db.update(schema.sessionNotes)
        .set({
          ...updateData,
          updated_at: new Date()
        })
      ...
      [sessionNote] = await db.insert(schema.sessionNotes)
        .values({
    ...
    if (isStudent && updateData.student_rating) {
      ...
      const totalRatings = allRatings.reduce((sum, item) => sum + Number(item.rating || 0), 0);
