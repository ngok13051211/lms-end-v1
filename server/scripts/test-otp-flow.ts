...
async function testOtpSystem() {
  ...
  try {
    ...
    if (!user) {
      console.log(
        `❌ Không tìm thấy người dùng với email ${TEST_EMAIL} trong database`
      );
    ...
    if (currentOtps.length > 0) {
      console.log(
        `🔍 Hiện có ${currentOtps.length} OTP trong database cho ${TEST_EMAIL}`
      );
      console.log(
        `🔍 OTP mới nhất được tạo lúc: ${new Date(
          currentOtps[0].created_at
        ).toLocaleString()}`
      );
    } else {
    ...
    try {
      ...
      console.log(
        `⚠️ OTP2 đã được tạo và gửi thành công mặc dù có rate limit: ${otp2}`
      );
      console.log("⚠️ Rate limiting có vẻ không hoạt động! Cần kiểm tra lại!");
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : "Unknown error";
      console.log(`✅ Rate limiting hoạt động tốt: ${errorMessage}`);
    }

    // Lấy OTP mới nhất để thử xác thực
    const latestOtp = await db.query.emailOtps.findFirst({
      where: and(eq(emailOtps.email, TEST_EMAIL), eq(emailOtps.used, false)),
      orderBy: (otps) => [desc(otps.created_at)],
    });
    ...
    console.log(
      "⚠️ Để test xác thực, vui lòng sử dụng OTP được log trong console ở bước trước."
    );
    console.log("⚠️ Bạn có thể sử dụng API để xác thực OTP thủ công:");
    console.log(
      `⚠️ POST /api/v1/verify/verify-otp với body: {"email": "${TEST_EMAIL}", "otp": "<mã otp>"}`
    );
    ...
    console.log(
      `🚫 Số OTP đã được sử dụng: ${finalOtps.filter((o) => o.used).length}`
    );
    console.log(
      `✅ Số OTP chưa sử dụng: ${finalOtps.filter((o) => !o.used).length}`
    );

    if (finalOtps.filter((o) => !o.used).length > 1) {
      console.log(
        "⚠️ CẢNH BÁO: Có nhiều hơn 1 OTP chưa sử dụng! Cần kiểm tra lại logic vô hiệu hóa OTP cũ."
      );
    } else {
